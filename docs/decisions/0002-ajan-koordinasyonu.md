...
- **Ekibin ikinci üyesi Notion'a yalnızca guest olarak eklenebiliyor.** Workspace bir edu hesabı üzerinden kuruldu; mevcut Notion paketi ek *member* eklemeye izin vermiyor. Guest erişimi sayfa bazındadır, workspace geneli değil — yani hub ağacının dışında oluşturulan hiçbir Notion nesnesi ikinci kişiye görünmez ve bu, protokolün sessizce tek taraflı çalışmasına yol açar.

  Bu yüzden `AGENTS.md` kural 8 tüm yeni Notion içeriğinin `duo-works` hub sayfası altında oluşturulmasını şart koşar: guest erişimi hub seviyesinde bir kez verilir, alt öğeler miras alır. Kısıt kabul edildi çünkü alternatifi (ücretli plana geçmek) bu aşamada gereksiz; bedeli, hub ağacı disiplininin artık mimari bir gereklilik olması.
